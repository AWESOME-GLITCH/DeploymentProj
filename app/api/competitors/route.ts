...
function demo(): CompetitorMatrix {
...
export async function POST(_req: NextRequest) {
  if (!hasLiveAgents()) return NextResponse.json({ matrix: demo(), demo: true, sources: [], searched: false });
  ...
  try {
    ...
    return NextResponse.json({ matrix, demo: false, sources: sink.sources || [], searched: Boolean(sink.searched) });
  } catch {
    return NextResponse.json({ matrix: demo(), demo: true, sources: [], searched: false, note: "Live agent errored; showing demo output." });
  }
}
