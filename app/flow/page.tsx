...
function ArtifactCard({ k, data }: { k: string; data: any }) {
  ...
  return (
    <Card glow="51,214,192" className="p-4">
      <div className="mb-2 flex items-center gap-2">
        <Icon name="Check" className="h-4 w-4 text-accent-teal" />
          ...
          {!loading && (artifacts || plan) && (
            <div className="animate-rise space-y-4">
              ...
              {artifacts &&
                ...
                Object.entries(artifacts).map(([k, v]) => <ArtifactCard key={k} k={k} data={v} />)}
            </div>
          )}
        </div>
      </div>
