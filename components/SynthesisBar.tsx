...
export type SynthItem = { tag: string; tone: "risk" | "pivot" | "ship" | "focus"; text: string; icon: string };

export const SYNTH_TONE: Record<SynthItem["tone"], { chip: string; dot: string }> = {
  risk: { chip: "bg-accent-rose/15 text-accent-rose", dot: "bg-accent-rose" },
  pivot: { chip: "bg-accent-blue/15 text-accent-blue", dot: "bg-accent-blue" },
  ship: { chip: "bg-accent-teal/15 text-accent-teal", dot: "bg-accent-teal" },
  focus: { chip: "bg-brand/15 text-brand-soft", dot: "bg-brand" },
};

/** Derives the rolling 3-bullet executive read from live state. */
export function useSynthesis(): SynthItem[] {
...
export function SynthesisBar() {
  ...
  if (!mounted || hidden || pathname === "/") return null;
        ...
        <div className="flex min-w-0 flex-1 gap-2 overflow-x-auto">
          {synthesis.map((s, i) => (
            <div key={i} className="flex min-w-0 shrink-0 items-center gap-2 rounded-full border border-line bg-bg-card px-2.5 py-1 md:shrink">
              <span className={`rounded px-1.5 py-0.5 text-[9px] font-bold ${SYNTH_TONE[s.tone].chip}`}>{s.tag}</span>
